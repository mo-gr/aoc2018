...
tick (score, (e0, e1)) = let
  r0 = score ! e0
  r1 = score ! e1
  next = singleton $ (r0 + r1) `mod` 10
  next' = if r0 + r1 >= 10 then singleton $ (r0 + r1) `div` 10 else empty
...
search target score c = let
  n = V.length target
  candidate = V.take n score
  in
    if target == candidate || V.null score then
      c
    else
      search target (V.tail score) (succ c)
...
solution1 = do
  let scoreBoard = start
      ...
      target = 293801
  pure $ unwrap target $ Prelude.head $ Prelude.drop (target + 10) $ iterate tick kitchen
