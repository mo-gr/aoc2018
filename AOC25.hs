...
unparse [] = ""
unparse (c:cs) = unparse cs ++ (unlines $ fmap (\s -> show (x s) ++ ","
        ++ show (y s) ++ ","
        ++ show (z s) ++ ","
        ++ show (t s)) (toList c))
...
filterSubSets' _ [] = []
filterSubSets' orig (a:aa) = case find (\a' -> (a `isProperSubsetOf` a')) orig of
...
mergeConstellations' cs = sort . filterSubSets $ toList $ fst (Prelude.foldl f (S.empty, S.empty)(liftM2 (,) cs cs))
  where
    f (acc, blacklist) (c,c') = if (c == c') then (acc, blacklist)
                   else if (c `member` blacklist) then (acc, blacklist)
                   else if (c' `member` blacklist) then (acc, blacklist)
                   else if (minimum (liftM2 distance (toList c) (toList c')) < 4) || (c `intersects` c')
                        then (S.insert (S.union c c') (S.delete c' $ S.delete c acc), S.insert c' $ S.insert c blacklist)
