...
import Data.Set as S (Set, unions, fromList, toList, empty, null, isSubsetOf, member, notMember, map, difference)
...
instance Ord Instruction where
  compare (Instruction step cond) (Instruction step' cond') =
    compare step step'
...
allPreconditions man s = case directPreconditions man s of
  [] -> mempty
...
isPrecondition man s s' = let 
  direct = isJust $ find (liftM2 (&&) ((== s)._cond) ((== s')._step)) man
  pres = directPreconditions man s' 
  in direct || (or $ isPrecondition man s <$> pres)

compareSteps :: [Instruction] -> Step -> Step -> Ordering
compareSteps man s s' =  let
  isPrecondition' = isPrecondition man
  ...
  in case (sFirst, s'First) of
    ...
    (True, False) -> LT
    (False, True) -> GT
    _ -> case (sHasNoPre, s'HasNoPre) of
      (True, False) -> LT
      (False, True) -> GT
      (False, False) -> EQ
...
validate [] s = Right s
validate (i:is) solution = let
  pidx = elemIndex (_cond i) solution
  sidx = elemIndex (_step i) solution
  in case (pidx, sidx) of
    (Just p, Just s) | p > s -> Left $ "rule invalid: " ++ show i
...
apply [] (b, s) = (b, s)
apply (i:is) (flipped, s) = let
  pIx = fromMaybe 0 $ elemIndex (_cond i) s
  sIx = fromMaybe 0 $ elemIndex (_step i) s
  in
    if pIx < sIx then apply is (flipped, s)
...
candidates man compl blocked = let 
  follows = S.fromList $ mconcat $ (\c -> directFollow man c) <$> S.toList compl
  availableFollows = S.filter (`S.member` blocked) follows
  in
    S.filter (\f -> allPreconditions man f `S.isSubsetOf` compl) availableFollows

next :: [Instruction] -> StateStep -> StateStep
next man (StateStep d [] []) = StateStep d [] []
next man (StateStep _ [] _) = error "Something went bad"
next man (StateStep c a b) = let
  c' = c ++ [(head.sort $ a)]
...
solution1 = do
  ...
  let steps = allSteps man
      ...
      initial = 
        StateStep [] starter $ filter (not . (`elem` starter)) steps
...
finished s =
  (S.null $ avail s)
  && (S.null $ block s)
  && (S.null $ busyWorkers s)
  || (currentTime s > 1000)
  
untilFinished :: SchedulerState -> Reader [Instruction] SchedulerState
untilFinished s | finished s = pure $ s
untilFinished !s = do 
  next <- tick s
  untilFinished next

initialScheduler :: [Step] -> Int -> SchedulerState
initialScheduler steps workerCount = let
  in SchedulerState mempty mempty (fromList steps) 0 (fromList $ (flip (,) $ Nothing) <$> [0..workerCount]) empty
...
nextAvailable compl blocked = do 
  ...
  pure $ S.filter (\f -> allPreconditions man f `S.isSubsetOf` complSet) availableFollows
...
tick s = do
  let updatedWorkers = S.map (fmap pred) (busyWorkers s)
      ...
      completed' = (completed s) <> (sort $ catMaybes $ snd <$> (S.toList finishedWorkers))
  ...
  pure $ SchedulerState completed' remainaing block' time' freeWorkers' busyWorkers'
...
scheduleWorkers freeWorkers availableSteps = 
  if S.null freeWorkers then
    (availableSteps, empty)
  else
    let w = toList freeWorkers
        s = sort $ toList availableSteps
        schedule = (\(w, step) -> (const (Just step) <$> w, duration step) ) <$> zip w s
