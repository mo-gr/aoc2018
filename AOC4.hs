...
data Message = WakeUp DateTime 
             | Sleep DateTime
             | ShiftStart DateTime GuardNum deriving (Show, Eq)
...
instance Show SleepLog where
  show (SleepLog g s w) = (show g) ++ ": " 
    ++ (show . todMin . dtTime $ s)
    ++ " - "
    ++ (show . todMin . dtTime $ w)
...
convertToSleepLog ms = snd $ foldr f (Nothing, []) (reverse $ sort ms)
  where
    f :: Message -> (Maybe (GuardNum, Maybe DateTime),[SleepLog]) -> (Maybe (GuardNum, Maybe DateTime),[SleepLog])
    f (ShiftStart _ guard) (_, ls) = (Just (guard, Nothing), ls)
    f (Sleep start) (Just (guard, Nothing), ls) = (Just (guard, Just start), ls)
    f (WakeUp wake) (Just (guard, Just start), ls) = (Just (guard, Nothing), (sleepLog guard start wake) : ls)
    f m s = error $  "unexpected message: " ++ show m ++ " state " ++ show s

sleepMinutes :: SleepLog -> Minutes
sleepMinutes s = let
  (Elapsed startTime) = timeGetElapsed . _sleep $ s
...
sleepTimeByGuard :: M.Map GuardNum Minutes -> [SleepLog] -> M.Map GuardNum Minutes
sleepTimeByGuard m [] = m
sleepTimeByGuard m (l:ls) = let
  mins = sleepMinutes l
  g = _guard l 
  in
...
findSleepiestMinutes log = let
  minutes :: [Minutes]
  minutes = enumFromTo (Minutes 0) (Minutes 59)
  filterLog :: [SleepLog] -> Minutes -> (Minutes, Int)
  filterLog l m = (m, length 
                      $ filter (\l -> (todMin . dtTime . _sleep $ l) <= m
                                   && (todMin . dtTime . _wake $ l) > m) l)
  in
    head . reverse . sortOn snd $ fmap (filterLog log) minutes


solution1 = do
  sleepLog <- convertToSleepLog <$> input
  sleepiestGuard <- pure . fst . head . reverse . sortOn snd . M.toList $ sleepTimeByGuard M.empty sleepLog
...
solution2 = do
  ...
  let sleepiest = (\l -> (_guard . head $ l, findSleepiestMinutes l)) <$> sleepLogByGuard
