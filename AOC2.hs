...
nSet _ [] = False
nSet n (x:xs) = case length $ filter (== x) xs of
  c | c == (n - 1) -> True
...
difference = difference' 0
  where
    ...
    difference' d other [] = d + length other
    difference' d (x:xs) (y:ys) = case x == y of
      True -> difference' d xs ys
      False -> difference' (d + 1) xs ys 
      
leastDiff :: [String] -> Maybe (String, String)
leastDiff [] = Nothing
leastDiff (x:xs) = case filter (\(x', d) -> d == 1) $ fmap (\x' -> (x', difference x x')) xs of
  [] -> leastDiff xs
  (x', _):_ -> Just (x, x')
