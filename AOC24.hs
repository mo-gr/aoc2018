...
instance Show Group where 
  show g = "Group " ++ show (initiative g) ++ " contains " ++ show (units g) ++ " units"
  
instance Eq Group where
  g == g' = (initiative g) == (initiative g')

instance Ord Group where
  compare g g' = case compare (effectivePower g) (effectivePower g') of
...
instance Show Strike where
  show (Strike p a t) = "Strike " ++ show (initiative a) ++ "->" 
                                  ++ show (initiative t) ++ " @ " 
                                  ++ show p

instance Ord Strike where 
  compare s s' = case compare (strikePower s) (strikePower s') of
    EQ -> case compare (effectivePower.target $ s) (effectivePower.target $ s') of
...
isImmune (Group {immune = gi}) sa =
  any (== sa) gi

isWeak :: Group -> Attack -> Bool
isWeak (Group {weak = gw}) sa =
  any (== sa) gw

strike :: Group -> Group -> Strike
strike attacker defender = let
  attackType = (attack attacker)
  damageMulti = if isWeak defender attackType 
                then (*2) 
...
performStrike s = let
  ...
  in
    if unitsAfterAttack <= 0 then
      Nothing
    else
      pure $ g{units = unitsAfterAttack}
...
select g targets = headMaybe . reverse . sort . filter ((> 0).strikePower) $ (strike g <$> targets)

dropTarget :: Strike -> [Group] -> [Group]
dropTarget (Strike {target = t}) g = filter (/= t) g

targetSelection :: BattleField -> [Strike]
targetSelection bf = let
  attackers = reverse . sort $ infection bf
  defenders = reverse . sort $ immuneSystem bf
  ts (strikes,defenders) a =  case select a defenders of
    Just s -> (s:strikes, dropTarget s defenders)
    Nothing -> (strikes, defenders)
  in
    sortOn (initiative.attacker) $ (fst $ foldl ts ([],defenders) attackers)
...
findArmies :: BattleField -> Strike -> (Maybe (Strike, [Group]), Maybe ([Group] -> BattleField))
findArmies bf s | elem (attacker s) (infection bf) &&
                    elem (target s) (immuneSystem bf) = let
                      strike' = strike (find' (attacker s) (infection bf)) (find' (target s) (immuneSystem bf))
                      in (Just (strike', immuneSystem bf), Just $ updateImmune bf)
                | elem (attacker s) (immuneSystem bf) &&
                    elem (target s) (infection bf) = let
                      strike' = strike (find' (attacker s) (immuneSystem bf)) (find' (target s) (infection bf))
...
updateArmies up g s = let t = (target s) in case performStrike s of
  Nothing -> up (filter (/= t) g)
  Just survivor -> up (map (\x -> if x == t then survivor else x) g)

performAttack :: BattleField -> Strike -> BattleField
performAttack bf strike = case findArmies bf strike of
  (Just (strike', defender), Just updater) -> updateArmies updater defender strike'
  _ -> bf
...
battle bf = let
  strikes = targetSelection bf
  in
    foldl performAttack bf $ reverse strikes
...
specialTratesParser = do
  _ <- string "("
  immune <- (string "immune to " *> (attackTypeParser `sepBy` (string ", ")))  <|> return []
  _ <- string "; " <|> return []
  weak <- (string "weak to " *> (attackTypeParser `sepBy` (string ", "))) <|> return []
  _ <- string "; " <|> return []
  immune' <- (string "immune to " *> (attackTypeParser `sepBy` (string ", ")))  <|> return []
  _ <- string ")" <* many space
...
battleFieldParser = do
  immune <- string "Immune System:" *> many space *> many groupParser <* many space
...
debugUntil cond f a | cond a = do print a; pure a
                    | otherwise = do
                        let a' = f a
                        print a
                        debugUntil cond f a'
...
boost bf b = bf {immuneSystem = (\g -> g {damage = b + damage g})<$> immuneSystem bf}
