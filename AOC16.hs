...
cpuParser = do
  ...
  return $ CPU (fromIntegral a) (fromIntegral b) (fromIntegral c) (fromIntegral d)
...
verify ((c, c', i):more) = do
  if instruct' c i == c' then
    verify more
  else
    print $ "Broken: " ++ show c ++ show i ++ " =/=> " ++ show c'  
...
solution1 = do
  ...
  let examples = case examplesOrError of
...
solution2 = do
  ...
  let (e,p) = case programOrError of
                   Left err -> error $ show err
