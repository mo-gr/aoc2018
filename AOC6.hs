...
newtype X = X { unX :: Int } deriving (Show, Eq, Num, Ord, Enum)
newtype Y = Y { unY :: Int } deriving (Show, Eq, Num, Ord, Enum)
...
distance (Coord x y) (Coord x' y') = unX (abs (x - x'))
                                   + unY (abs (y - y')) 
...
infinity = do
  ...
  pure $ [Coord x' y' | x' <- [(x zero' - 1)..(x finity' + 1)],
...
world = do
  zero' <- zero
  finity' <- finity
  pure $ [Coord x' y' | x' <- [(x zero')..(x finity')],
                        y' <- [(y zero')..(y finity')]]

findClosest :: [Coord] -> Coord -> Maybe Coord
findClosest candidates point = firstOrNothing $ (sortOn snd) $ zip candidates $ distance point <$> candidates
  where
    firstOrNothing ((_, x) : ((_, y) : more)) | x == y = Nothing
